...
class ControlNode(Node):
    ...
    def waypoint_callback(self, msg):
        ...
        #caso esteja tranquilo, calcula o angulo, envia o movimento e envia os dados deste pela porta serial
        steering_angle = self.calculate_steering_angle(xm, ym)

        self.send_command(steering_angle, state = 1, power = CONSTANT_THRUST)
        self.send_telemetry(xm, ym, CONSTANT_THRUST)
    ...
    def calculate_steering_angle(self, xm,ym):
        ...
        #evitamos passar do limite e, caso o waypoint esteja na frente, simplesmente o angulo continua sendo 90 graus
        displacement_angle = max(STEERING_MIN, min(STEERING_MAX, STEERING_CENTER + delta_pursuitDEG))
        #SE O CARRO VIRAR PRO LADO ERRADO: devo trocar o sinal de ym ou de alpha
    ...
    def send_command(self, angleDEG, state, power):
        ...
        state = str(state)
        #apenas por segurança também, já que no momento estamos lidando com potência constante
    ...
    def send_telemetry(self, xm, ym, power):
        #apenas uma aproximação dado que não tem PD
        speed = power
        message = {"speed": speed, "acceleration": 0.0, "engine": True}
